...
// updated for pricing plans

require("dotenv").config();
const stripe = require("stripe")(process.env.STRIPE_SECRET_KEY, { apiVersion: "2023-10-16" });
const User = require("../Models/User");

// For plan comparisons and logic
const planOrder = ["free", "basic", "standard", "premium"];

function getPlanIndex(plan) {
  return planOrder.indexOf((plan || "free").toLowerCase());
}
...
exports.subscribeUser = async (req, res) => {
  ...
  try {
    ...
    const userPlanIdx = getPlanIndex(user.plan);
    const requestedPlanIdx = getPlanIndex(plan);

    // Prevent downgrades to a lower plan
    if (requestedPlanIdx < userPlanIdx) {
      return res.status(400).json({ error: "Cannot downgrade to a lower plan until your current subscription expires." });
    }
    ...
    // 🔴 TEMPORARY DB update — for UI feedback only, replaced by webhook logic after payment
    // user.isPremium = true;
    // user.plan = plan;
    // user.expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // Approximate, replaced by webhook
    // await user.save();
    // console.log("User after save (temp update):", user);
